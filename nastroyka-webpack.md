...
Все шрифты нашего проекта будут располагаться в каталоге `src/fonts/`.

Для начала скачаем шрифты и CSS для их подключения.
...
1. На странице [google-webfonts-helper](https://google-webfonts-helper.herokuapp.com/fonts) в списке слева нужно выбрать необходимый шрифт.
   - Справа от списка появится страница для выбранного шрифта. На этой странице можно дополнительно указать нужные кодировки и стили, которые планируется использовать для данного шрифта.
   - В секции `Copy CSS:` в поле `Customize folder prefix (optional):` пропишите путь `fonts/` и скопируйте из серого поля CSS-код в файл `src/main.scss`.
...
Обычно, рекомендуется оптимизировать шрифты, сжимая их на таких сервисах как [Font2web](https://www.font2web.com/) или [Transfonter](https://transfonter.org/), но шрифты от Google оптимизированы.

Для обработки шрифтов добавим следующие правила в файл `webpack.config.js`:
...
В этой статье показано, как настроить вебпак для того, чтобы создавать фронтенд сайта используя шаблонизатор Pug, препроцессор SCSS, язык разметки Markdown и язык JavaScript. Показаны методы оптимизации изображений, шрифтов, настройка локального сервера.
