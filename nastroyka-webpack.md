...
- [О чем статья](#о-чем-статья)
- [Полезные ресурсы для настройки webpack](#полезные-ресурсы-для-настройки-webpack)
- [Быстрый запуск Webpack](#быстрый-запуск-webpack)
- [Установка и настройка html-webpack-plugin](#установка-и-настройка-html-webpack-plugin)
- [Установка и настройка DevServer](#установка-и-настройка-devserver)
- [Настройка в Webpack точки входа и выхода](#настройка-в-webpack-точки-входа-и-выхода)
- [Точка входа](#точка-входа)
- [Точка выхода](#точка-выхода) -[Автоматическая очистка каталогов](#автоматическая-очистка-каталогов)
- [Настройка режима production в webpack](#настройка-режима-production-в-webpack)
- [Babel](#babel)
- [Подключение шаблонизатора Pug](#подключение-шаблонизатора-pug)
- [Загрузка стилей](#загрузка-стилей-в-webpack)
- [Загрузка изображений](#загрузка-изображений-в-webpack)
- [Оптимизация изображений](#оптимизация-изображений)
- [Включение синтаксиса Markdown и файлов .md в Pug](#включение-синтаксиса-markdown-и-файлов-md-в-pug)
...
В статье подробно описывается настройка сборщика Webpack 5. Слово "подробно" означает, что здесь будут, по возможности, объясняться все непонятные моменты в настройках. Статья покажет, как создать рабочий шаблон, который позволит создавать фронтенд сайта и использовать для этого следующие инструменты: Pug, Sass, JavaScript и Markdown.
...
Дальнейшие действия выполнялись в ОС Ubuntu 20.04 с NodeJS v16.17.1 и NPM v8.15.0, использовался стандартный Терминал, стандартный Текстовый редактор (далее по тексту будет называться Редактор кода) и браузер FireFox v105.0. Данная строка не обязательна к выполнению, а лишь показывает, какие инструменты использовал автор.
...
Затем в редакторе кода откроем `package.json` и добавим в поле `"scripts"` следующую строку кода:
...
В терминале запустим следующую команду:
...
Мы создали объект `module`, для которого задали правило `rules`. Для вебпака файл - это модуль, будь то скрипт, стили, шрифт или изображение. Здесь, для всех модулей (читай, файлов) с расширением `.js` webpack будет применять плагин `babel-loader`. Правило не действует на каталог `node_modules`, что указывается в свойстве `exclude`
...
В параграфе [Автоматическая очистка каталогов](#автоматическая-очистка-каталогов) применялся плагин `filemanager-webpack-plugin`, который может удалять, копировать, создавать, перемещать и архивировать файлы и каталоги, как перед началом сборки, так и по ее окончании. Применим этот плагин для копирования ресурсов. Так как плагин в проекте уже установлен, то остается его настроить для копирования.
...
В этой статье показывается, как настроить вебпак для создания фронтенда сайта с использованием шаблонизатора Pug, препроцессора SCSS, языка разметки Markdown и языка JavaScript. Показываются методы оптимизации изображений, подключения шрифтов, настройки локального сервера.
