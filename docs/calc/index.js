...
function App() {
  ...
  var handleKey = function handleKey(symbol) {
    ...
    switch (symbol) {
      ...
      case digit:
        if (
          stateValue.curVal.length < 22 &&
          stateValue.curVal !== "Digit Limit Met"
        ) {
          setAll({
            allVal:
              stateAll.allVal === "0" || /=/g.test(stateAll.allVal) //если выражение равно нулю или содержит знак равно
          ...
          setValue({
            curVal:
              stateValue.curVal === "0" ||
              /[x/+-]/.test(stateValue.curVal) ||
              /=/g.test(stateAll.allVal)
                ? symbol
                : stateValue.curVal + symbol,
          })
        } else {
      ...
      case ".":
        ...
        setValue({
          curVal: /=/g.test(stateAll.allVal) //если в выражении есть знак "равно"
            ...
            ? "0."
            : /\./g.test(stateValue.curVal) //то на место оператора ставится ноль с точкой, иначе, проверяется наличие точки
            ? stateValue.curVal
            : stateValue.curVal + ".", //если точка есть, то значение не меняется, если же точки нет, то к значению добавляется точка
        })
        break
      case "=":
        setAll({
          allVal:
            /[=]/g.test(stateAll.allVal) || /[x/+-]$/.test(stateAll.allVal) //если в выражении есть "=" или в конце выржения присутствует любой из операторов "x", "/", "+" или "-"
        ...
        setValue({
          curVal:
            /[=]/g.test(stateAll.allVal) ||
            /[x/+-]/.test(stateValue.curVal) ||
            stateValue.curVal === "0." //если в выражении есть "=", или если в текущем значении есть любой из операторов "x", "/", "+", "-", или текущее значение равно нулю с точкой
    ...
    function backspace(arg1) {
      var arg2 =
        arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : arg1
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        {
          target: "_blank",
          href: "https://github.com/injashkin/Javascript-Calculator",
        },
        "GitHub."
      ),
      React.createElement("br", null),
...
function Display(props) {
  return React.createElement(
    ...
    React.createElement("div", { className: "expression" }, props.allVal),
    React.createElement(
      "div",
