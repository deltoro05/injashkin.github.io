...
- Разметка Markdown полезна для наполнения текстовой информацией страниц сайта. Разметка Markdown преобразуется в понятный для браузеров HTML.
...
В редакторе откроем файл `webpack.config.js` и вставим следующий код:
...
- `module.exports = {}` - все настройки файла `webpack.config.js` помещаются в объект, который экспортируется как модуль по умолчанию.
...
- `__dirname` - глобальная константа, указывающая абсолютный путь к каталогу файла, в котором эта константа используется. В нашем случае, константа `__dirname` используется в файле `webpack.config.js`, который находится в корневом каталоге проекта, поэтому `__dirname` содержит абсолютный путь к корневому каталогу проекта.
...
3. Из терминала запускаем сборку проекта следующей командой:
...
`serve` означает запустить веб сервер. [`--open`](https://webpack.js.org/configuration/dev-server/#devserveropen) автоматически запускает браузер. `--mode development` включает режим разработки.

Перед тем как запустить веб-сервер удалим каталог `dist`. Это делать необязательно, а нужно для понимания работы веб-сервера.

Из терминала запустим веб-сервер командой:
