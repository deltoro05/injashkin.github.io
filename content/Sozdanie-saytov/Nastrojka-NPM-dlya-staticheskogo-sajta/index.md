...
Для написания стилей мы будем использовать препроцессор [Sass](https://github.com/sass/node-sass#command-line-interface). Он позволяет нам использовать вложенности, миксины, конструкции `if/else`, операторы сравнения, логические операторы. Такие возможности, как импорты, переменные, наследование, математические операторы используются не только в SCSS, но и в CSS, однако преимущество препроцессоров в том, что эти функции в препроцессорах компилируются один раз на этапе сборки проекта, а в CSS каждый раз выполняются браузером, что в некоторых случаях сказывается на производительности.
