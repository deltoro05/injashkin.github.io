...
Загрузите последнюю версию WordPress с [официального сайта](https://wordpress.org/latest.zip), либо его [русифицированную версию](https://ru.wordpress.org/latest-ru_RU.zip).

Распакуйте скачанный архив в `Домашнюю папку`. В результате появится каталог `wordpress`, который переименуйте в `wp2` (просто wp2 короче, чем wordpress).
...
2. Нажмите кнопку `Вперед!` Появится следующее окно, в котором заполните поля согласно скриншоту:
...
4. Нажмите кнопку `Запустить установку`, появится форма, в которой введите название сайта и данные администратора согласно скриншоту:
...
Каждая версия [Wordless](https://github.com/welaika/wordless_gem) работает с определенной версией Node. Последний Wordless версии 2.2.0 требует для своей работы Node v14.15.3.
...
Для того, чтобы установить [Wordless](https://wordless.readthedocs.io/en/latest/index.html) требуется язык Ruby. Проверьте наличие этого языка в системе:
