...
- `git commit -a -m 'added new'`
  - параметр `-a` заставляет Git автоматически индексировать каждый уже отслеживаемый на момент коммита файл, позволяет обойтись без `git add`
  
---

- `git commit --amend`
  - откроется редактор с последним коммитом, в котором можно исправить комментарий к коммиту. Если после последнего коммита был сделан push, то нужно выполнить еще одну команду: `git push <remote> <branch> --force`
...
- `git branch -m old new`
  - переименовывает локальную ветку с именем `old` на `new`. 
