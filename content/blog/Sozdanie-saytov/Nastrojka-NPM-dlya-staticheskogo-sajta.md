...
Затем создадим каталог `content`, а в нем еще пару каталогов `article1` и `article2`. В каждом из этих двух каталогов создадим по одному файлу `index.md` со следующим содержимым.
...
Для этого, в каталоге `src` создайте каталог `pages`, а в нем каталог `article`, где создайте файл `index.pug` со следующим содержимым:
